...
def compare_algorithms(X, y, estimators, measure_name, copula=None, regression=True):
    alg_accuracy_scores = {}
    ...
    alg_accuracy_scores['backward'] = algos.backward_selection(X, y, 6, measure, 
            estimators, cv=10, regression=regression, copula=copula)
...
def compare_kernels(X, y, estimators, measure_name, copula=None, regression=True):
    ...
    kern_accuracy_scores['rbf'] = algos.heuristic_selection(X, y, 6, measure, 
            estimators, regression=regression, copula=copula)
